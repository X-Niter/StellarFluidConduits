...
public class FluidSettings extends BaseSettingsPanel {    static final int ID_REDSTONE_BUTTON = GuiExternalConnection.nextButtonId();
    ...
    public FluidSettings(@Nonnull final IGuiExternalConnection gui, @Nonnull IClientConduit con) {
        super(IconEIO.WRENCH_OVERLAY_GAS, FluidConduitObject.itemFluidConduit.getUnlocalisedName(), gui, con, "in_out_settings");

        conduit = (ILiquidConduit) con;
        if (con instanceof StellarFluidConduit) {
            isEnder = true;
            eCon = (StellarFluidConduit) con;
        }
    ...
    public void actionPerformed(@Nonnull GuiButton guiButton) {
        ...
        if (isEnder) {
            PacketHandler.INSTANCE.sendToServer(new PacketStellarFluidConduit(eCon, gui.getDir()));
        }
    }
    ...
    private void createGhostSlots() {
        NNList<ItemStack> filtersAll = new NNList<>(new ItemStack(FluidConduitObject.itemFluidFilter.getItemNN()));
